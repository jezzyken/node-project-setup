...
const checkPermission = (resource, action) => {
  return async (req, res, next) => {
    try {
      ...
      const actionPermission = permission.actions.find(a => a.name === action);
...
const autoCheckPermission = async (req, res, next) => {
  try {
    ...
    // Check if user is accessing their own account (self-access exception)
...
module.exports = { checkPermission, autoCheckPermission };